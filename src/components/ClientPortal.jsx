...
export default function ClientPortal() {
  ...
  return <div className="portal-page"><PortalHeader session={session} onSignOut={async () => { await apiRequest('/api/client/logout', { method: 'POST' }); window.location.reload() }} /><main className="portal-content"><div className="portal-title"><div><span className="portal-eyebrow">Client portal</span><h1>Your secure workspace.</h1><p>Send files directly to the Overdrive team. Documents are stored in a private Cloudflare R2 bucket.</p></div><label className="upload-button"><UploadCloud size={17} />{uploading ? 'Uploading...' : 'Upload document'}<input type="file" onChange={uploadDocument} disabled={uploading} /></label></div>{state.error && <p className="form-error">{state.error}</p>}<section className="documents-panel"><div className="panel-heading"><div><span className="portal-eyebrow">Published files</span><h2>Your documents</h2></div><span>{documents.length} files</span></div>{documents.length === 0 ? <div className="empty-state"><FileText size={28} /><p>No published documents yet. The Overdrive team will post your prepared files here.</p></div> : <div className="document-list">{documents.map((document) => <div className="document-row" key={document.id}><span className="document-icon"><FileText size={19} /></span><span><strong>{document.file_name}</strong><small>{document.category || 'Other'} - {new Date(document.created_at).toLocaleDateString()}</small></span><span className="document-row-actions"><a href={`/api/client/documents/${document.id}/download`} target="_blank" rel="noreferrer" aria-label={`View ${document.file_name}`}><Eye size={15} /> View</a><a href={`/api/client/documents/${document.id}/download?download=1`} aria-label={`Download ${document.file_name}`}><Download size={15} /> Download</a></span></div>)}</div>}</section></main></div>
}
