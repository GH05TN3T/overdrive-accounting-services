...
import { ArrowUpRight, CalendarDays, CheckCircle2, Clock3, Download, ExternalLink, Eye, FileText, FolderOpen, LayoutDashboard, LoaderCircle, LogOut, Mail, RefreshCw, Save, Search, Settings, Trash2, UploadCloud, Users, Video, X } from 'lucide-react'
...
export default function AdminDashboard() {
  ...
  return <DashboardFrame session={session} activeView={activeView} onNavigate={navigateDashboard} onSignOut={async () => { await apiRequest('/api/admin/logout', { method: 'POST' }); window.location.reload() }}>
    ...
    <div className={`dashboard-grid ${activeView === 'overview' || activeView === 'appointments' || activeView === 'documents' ? '' : 'panel-hidden'}`}>
      <section className={`dashboard-panel appointments-panel ${activeView === 'overview' || activeView === 'appointments' ? '' : 'panel-hidden'}`} id="appointments"><div className="panel-heading"><div><span className="portal-eyebrow">Live queue</span><h2>Appointments</h2></div><span className="live-pill"><span /> Auto refresh</span></div><div className="table-tools"><div className="search-box"><Search size={16} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search clients" /></div><div className="filter-tabs">{filters.map((item) => <button className={filter === item ? 'active' : ''} key={item} onClick={() => setFilter(item)}>{item}</button>)}</div></div><div className="appointment-list">{visibleAppointments.length === 0 ? <div className="empty-state"><CalendarDays size={28} /><p>No appointments match this view.</p></div> : visibleAppointments.map((appointment) => <div className="appointment-row" key={appointment.id}><div className="appointment-date"><strong>{new Date(`${appointment.appointment_date}T12:00:00`).toLocaleDateString('en-US', { day: '2-digit' })}</strong><span>{new Date(`${appointment.appointment_date}T12:00:00`).toLocaleDateString('en-US', { month: 'short' })}</span></div><div className="appointment-details"><strong>{appointment.name}</strong><span>{appointment.business || 'Individual client'} - {appointment.service}</span><small>{appointment.email}{appointment.appointment_time ? ` - ${appointment.appointment_time}` : ''}</small></div><select className={`status-select status-${appointment.status}`} value={appointment.status} onChange={(event) => updateStatus(appointment.id, event.target.value)} aria-label={`Update status for ${appointment.name}`}><option value="new">New</option><option value="confirmed">Confirmed</option><option value="completed">Completed</option><option value="cancelled">Cancelled</option></select><button className="manage-appointment" onClick={() => openAppointment(appointment)}>Manage</button></div>)}</div></section>
      <section className={`dashboard-panel documents-panel-admin ${activeView === 'overview' || activeView === 'documents' ? '' : 'panel-hidden'}`} id="documents"><div className="panel-heading"><div><span className="portal-eyebrow">Cloudflare R2</span><h2>Client documents</h2></div><span>{documents.length} total</span></div><div className="document-actions"><div className="document-search"><Search size={15} /><input value={documentSearch} onChange={(event) => setDocumentSearch(event.target.value)} placeholder="Search files or clients" /></div><input className="document-client-email" type="email" value={documentClientEmail} onChange={(event) => setDocumentClientEmail(event.target.value)} placeholder="Client email for upload" /><select className="document-category" value={documentCategory} onChange={(event) => setDocumentCategory(event.target.value)}><option>Tax document</option><option>Payroll</option><option>Accounting</option><option>Other</option></select><label className="document-publish-check"><input type="checkbox" checked={publishDocument} onChange={(event) => setPublishDocument(event.target.checked)} /> Publish now</label><label className="document-upload-button"><UploadCloud size={15} />{uploadingDocument ? 'Uploading...' : 'Upload file'}<input type="file" onChange={uploadDocument} disabled={uploadingDocument} /></label></div>{visibleDocuments.length === 0 ? <div className="empty-state"><FileText size={28} /><p>No documents match this search.</p></div> : <div className="admin-document-list">{visibleDocuments.slice(0, 10).map((document) => <div className="admin-document-row" key={document.id}><span className="document-icon"><FileText size={18} /></span><span className="admin-document-info"><strong>{document.file_name}</strong><small>{document.client_email} - {document.category || 'Other'}</small></span><span className="document-row-actions"><a href={`/api/admin/documents/${document.id}/download`} target="_blank" rel="noreferrer" aria-label={`View ${document.file_name}`}><Eye size={15} /> View</a><a href={`/api/admin/documents/${document.id}/download?download=1`} aria-label={`Download ${document.file_name}`}><Download size={15} /> Download</a></span><span className={`document-state ${document.is_published ? 'published' : 'draft'}`}>{document.is_published ? 'Published' : 'Draft'}</span><button className="publish-document" onClick={() => toggleDocumentPublish(document)}>{document.is_published ? 'Unpublish' : 'Publish'}</button><button className="delete-document" onClick={() => deleteDocument(document)} aria-label={`Delete ${document.file_name}`}><Trash2 size={15} /></button></div>)}</div>}</section>
    </div>
    <section className={`dashboard-panel calendar-panel ${activeView === 'calendar' ? '' : 'panel-hidden'}`} id="calendar"><CalendarView appointments={appointments} onSelectAppointment={openAppointment} /></section>
    <div className={activeView === 'clients' ? '' : 'panel-hidden'}><ClientManagement documents={documents} /></div>
