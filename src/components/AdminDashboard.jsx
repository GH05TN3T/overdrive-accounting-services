...
import { ArrowUpRight, BarChart3, CalendarDays, CheckCircle2, Clock3, ExternalLink, FileText, FolderOpen, LayoutDashboard, LoaderCircle, LogOut, RefreshCw, Search, Settings, Trash2, Users } from 'lucide-react'
...
export default function AdminDashboard() {
  ...
  return <DashboardFrame session={session} onSignOut={async () => { await apiRequest('/api/admin/logout', { method: 'POST' }); window.location.reload() }}><div className="dashboard-heading" id="overview"><div><span className="portal-eyebrow">Cloudflare operations</span><h1>Good morning, Overdrive.</h1><p>Appointments and client documents, together in one place. Data refreshes every 30 seconds.</p></div><button className="dashboard-refresh" onClick={() => loadDashboard(true)}><RefreshCw className={refreshing ? 'spin' : ''} size={16} /> Refresh</button></div><div className="dashboard-stats"><Stat icon={<CalendarDays />} label="Total appointments" value={appointments.length} /><Stat icon={<Clock3 />} label="Needs review" value={newCount} accent /><Stat icon={<Users />} label="Client documents" value={documents.length} /><Stat icon={<CheckCircle2 />} label="Completed" value={appointments.filter((appointment) => appointment.status === 'completed').length} /></div>{state.error && <p className="form-error dashboard-error">{state.error}</p>}<div className="dashboard-grid"><section className="dashboard-panel appointments-panel" id="appointments"><div className="panel-heading"><div><span className="portal-eyebrow">Live queue</span><h2>Appointments</h2></div><span className="live-pill"><span /> Auto refresh</span></div><div className="table-tools"><div className="search-box"><Search size={16} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search clients" /></div><div className="filter-tabs">{filters.map((item) => <button className={filter === item ? 'active' : ''} key={item} onClick={() => setFilter(item)}>{item}</button>)}</div></div><div className="appointment-list">{visibleAppointments.length === 0 ? <div className="empty-state"><CalendarDays size={28} /><p>No appointments match this view.</p></div> : visibleAppointments.map((appointment) => <div className="appointment-row" key={appointment.id}><div className="appointment-date"><strong>{new Date(`${appointment.appointment_date}T12:00:00`).toLocaleDateString('en-US', { day: '2-digit' })}</strong><span>{new Date(`${appointment.appointment_date}T12:00:00`).toLocaleDateString('en-US', { month: 'short' })}</span></div><div className="appointment-details"><strong>{appointment.name}</strong><span>{appointment.business || 'Individual client'} · {appointment.service}</span><small>{appointment.email}</small></div><select className={`status-select status-${appointment.status}`} value={appointment.status} onChange={(event) => updateStatus(appointment.id, event.target.value)} aria-label={`Update status for ${appointment.name}`}><option value="new">New</option><option value="confirmed">Confirmed</option><option value="completed">Completed</option><option value="cancelled">Cancelled</option></select></div>)}</div></section><section className="dashboard-panel documents-panel-admin" id="documents"><div className="panel-heading"><div><span className="portal-eyebrow">Cloudflare R2</span><h2>Recent documents</h2></div><span>{documents.length} total</span></div>{documents.length === 0 ? <div className="empty-state"><FileText size={28} /><p>Client documents will appear here after upload.</p></div> : <div className="admin-document-list">{documents.slice(0, 8).map((document) => <div className="admin-document-row" key={document.id}><a href={`/api/admin/documents/${document.id}/download`}><span className="document-icon"><FileText size={18} /></span><span><strong>{document.file_name}</strong><small>{document.client_email}</small></span></a><button className="delete-document" onClick={() => deleteDocument(document)} aria-label={`Delete ${document.file_name}`}><Trash2 size={15} /></button></div>)}</div>}</section></div></DashboardFrame>
}
...
function DashboardFrame({ children, session, onSignOut }) { return <div className="dashboard-page dashboard-shell"><aside className="dashboard-sidebar"><a href="/" className="portal-logo"><img src="https://overdriveaccountingservices.com/wp-content/uploads/2024/05/WebLogo_White.png" alt="Overdrive Accounting Services" /></a><div className="sidebar-label">Workspace</div><nav className="dashboard-nav"><a className="active" href="/admin#overview">Overview</a><a href="/admin#appointments" onClick={(event) => { event.preventDefault(); document.getElementById('appointments')?.scrollIntoView({ behavior: 'smooth', block: 'start' }) }}><CalendarDays size={16} /> Appointments</a><a href="/admin#documents" onClick={(event) => { event.preventDefault(); document.getElementById('documents')?.scrollIntoView({ behavior: 'smooth', block: 'start' }) }}><FolderOpen size={16} /> Documents</a><a href="/"><ExternalLink size={16} /> Public website</a></nav><div className="sidebar-footer"><span><span className="status-dot" /> Systems online</span><small>D1 + R2 workspace</small><a href="mailto:Info@OverdriveAccountingServices.com"><Settings size={14} /> Get support</a></div></aside><div className="dashboard-main"><header className="dashboard-header"><div className="dashboard-breadcrumb"><span>Overdrive</span><strong>Operations</strong></div><div className="dashboard-user">{session?.email && <><span>{session.email}</span><button onClick={onSignOut}><LogOut size={15} /> Sign out</button></>}</div></header><main className="dashboard-content">{children}</main></div></div> }
