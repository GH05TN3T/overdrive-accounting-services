...
export default function AdminDashboard() {
  ...
  if (!session?.authenticated || !session?.isAdmin) return <DashboardFrame><AccessRequired /></DashboardFrame>
...
function AccessRequired() { return <div className="admin-login"><span className="portal-eyebrow">Cloudflare Access required</span><h1>Admin sign in.</h1><p>Protect `/admin*` and `/api/admin/*` with a Cloudflare Access application. Once your email is verified, reload this page to open the dashboard.</p><a className="button button-accent" href="/">View public website <ArrowUpRight size={16} /></a></div> }
function DashboardFrame({ children, session }) { return <div className="dashboard-page"><header className="dashboard-header"><a href="/" className="portal-logo"><img src="https://overdriveaccountingservices.com/wp-content/uploads/2024/05/WebLogo_White.png" alt="Overdrive Accounting Services" /></a><div className="dashboard-user">{session?.email && <><span>{session.email}</span><span><LogOut size={15} /> Cloudflare Access</span></>}</div></header><main className="dashboard-content">{children}</main></div> }
